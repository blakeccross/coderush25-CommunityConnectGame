            ...
            <CardHeader className="text-center space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xl font-semibold">
                  {playerRank === 1
                    ? "🏆 You Won!"
                    : `${playerRank}${playerRank === 2 ? "nd" : playerRank === 3 ? "rd" : "th"} Place`}
                </p>
              </div>
            </CardHeader>
            <CardContent>
              <Button onClick={handleEndGame} className="w-full text-lg h-12 font-semibold" size="lg">
                Back to Home
              </Button>
            </CardContent>
          </Card>
      ...
      <div className="min-h-screen p-4 bg-gradient-to-br from-primary/10 via-secondary/10 to-accent/10">
        <div className="max-w-md mx-auto space-y-6 py-8">
          <Card className="shadow-xl">
            <CardHeader className="text-center space-y-4">
              <div className="flex justify-center">
                <div className={`p-4 rounded-full ${isCorrect ? "bg-accent/20" : "bg-destructive/20"}`}>
                  {isCorrect ? (
              ...
              <CardTitle className="text-3xl font-bold">{isCorrect ? "Correct!" : "Incorrect"}</CardTitle>
              {isCorrect && pointsEarned > 0 && (
                <p className="text-2xl font-bold text-accent">+{pointsEarned} points</p>
              )}
            </CardHeader>
      ...
      <div className="max-w-md mx-auto space-y-6 py-8">
        <Card className="shadow-xl">
          <CardHeader className="space-y-4">
            ...
            <p className="text-xl font-bold text-center py-4 text-balance">{currentQuestion.question}</p>
          </CardHeader>
          <CardContent className="space-y-4">
            {hasSubmitted ? (
              <div className="text-center py-8 space-y-4">
                ...
                <p className="text-lg font-semibold">Answer Submitted!</p>
              ...
              <div className="grid gap-3">
                {currentQuestion.answers.map((answer, index) => (
                  <Button
                    ...
                    variant="outline"
                    className="h-auto py-4 px-6 text-left justify-start text-base font-medium hover:bg-primary hover:text-primary-foreground transition-colors"
                  >
